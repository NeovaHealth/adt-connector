...
object TestVars {
  ...
  final val patientThreeId = randomId()

  val PID3 = "PID|1|^^^^PAS||" + patientThreeId + "|DUMMY^PATIENT HEIDI^^^Ms||19740613000000|F||||||||||||||B^White Irish|||||||\"\"|N"

  val WARD = "E8"

  val visitID = randomId()

  val A05Dup     = "MSH|^~\\&|iPM|iIE|T4skr|T4skr|20131007152356.695+0100||ADT^A05|609299|T|2.4\r" +
    PID3 + "\r"+
