...
class PatientNewUpdateMergeDischargeTest extends ADTTest{
  ...
  val visitNew        = "MSH|^~\\&|iPM|iIE|Wardware|Wardware|20120105105702||ADT^A01|607639|P|2.4|||AL|AL\r" +
                        ...
                        PID2 +"\r" +
                        "PV1|1|I|E8^^^^^^^^Cobham Clinic|11||^^^^^^^^|^^^^^|C6035630^Ahmed^R^^^Dr|C5205403^Abdunabi^M^^^Mr|110|||||||C5205403^Abdunabi^M^^^Mr|01|1063654|||||||||||||||||||||||||20120103090000\r" +
  ...
  def testAddUpdateMerge(){
    ...
    sendMessageAndExpectResponse(patientMerge, "MSA|AA|")
    log.info("new visit for patientTwo")
    sendMessageAndExpectResponse(visitNew, "MSA|AA|")
